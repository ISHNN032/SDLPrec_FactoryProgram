...
SDL_Renderer *acc_screen = NULL;
SDL_bool retval = SDL_FALSE;
SDL_bool done = SDL_FALSE;

AccActivity::AccActivity(int){}
AccActivity::~AccActivity(){}

void AccActivity::DrawRect(SDL_Renderer *r, const int x, const int y, const int w, const int h)
{
    const SDL_Rect area = { x, y, w, h };
    SDL_RenderFillRect(r, &area);
}

void AccActivity::loop(void *arg)
{
    SDL_Event event;
    int i;
    ...
    SDL_SetRenderDrawColor(acc_screen, 0x0, 0x0, 0x0, SDL_ALPHA_OPAQUE);
        SDL_RenderClear(acc_screen);
        ...
        SDL_SetRenderDrawColor(acc_screen, 0x00, 0xFF, 0x00, SDL_ALPHA_OPAQUE);
        for (i = 0; i < SDL_JoystickNumButtons(joystick); ++i) {
            if (SDL_JoystickGetButton(joystick, i) == SDL_PRESSED) {
                DrawRect(acc_screen, (i%20) * 34, SCREEN_HEIGHT - 68 + (i/20) * 34, 32, 32);
            }
        }

        SDL_SetRenderDrawColor(acc_screen, 0xFF, 0x00, 0x00, SDL_ALPHA_OPAQUE);
        for (i = 0; i < SDL_JoystickNumAxes(joystick); ++i) {
            ...
            DrawRect(acc_screen, x, y, 16, 16);
        }

        SDL_SetRenderDrawColor(acc_screen, 0x00, 0x00, 0xFF, SDL_ALPHA_OPAQUE);
        for (i = 0; i < SDL_JoystickNumHats(joystick); ++i) {
            ...
            DrawRect(acc_screen, x, y, 8, 8);
        }

        SDL_RenderPresent(acc_screen);
...
SDL_bool AccActivity::WatchJoystick(SDL_Joystick * joystick)
{
    ...
    acc_screen = SDL_CreateRenderer(window, -1, 0);
    if (acc_screen == NULL) {
    ...
    SDL_SetRenderDrawColor(acc_screen, 0x00, 0x00, 0x00, SDL_ALPHA_OPAQUE);
    SDL_RenderClear(acc_screen);
    SDL_RenderPresent(acc_screen);
    ...
    SDL_DestroyRenderer(acc_screen);
    acc_screen = NULL;
    SDL_DestroyWindow(window);
    return retval;
}

void AccActivity::start(char *argv[])
{
